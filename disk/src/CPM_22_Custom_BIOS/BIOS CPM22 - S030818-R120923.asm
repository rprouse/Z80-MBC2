;==============================================================================
;
; Z80-MBC2 (HW ref: A040618) CP/M 2.2 Custom BIOS (CBIOS) - S030818-R120923
;
;
; NOTE: This CBIOS requires IOS S220718-R290823 (or following revisions unless stated otherwise)
;
;
...
; S030818-R140319   Added CONST MODE to select 7 or 8 bit mode (for XMODEM).
; S030818-R120923   Added support for the SPP Adapter board (A240721-R270921 and
;                    following revisions unless stated otherwise) as CP/M "list" device.
;                   Now the "CP/M WARM BOOT..." message is displayed during a warm boot only if 
;                    previously enabled by the SETOPT Opcode (IOS).
;
;==============================================================================
...
IOBYT       = $0003          ; IOBYTE address (IOBYTE not implemented/used)
...
READER
    jp READER_  ; READER CHARACTER IN
HOME
...
; --------------------------------------------------------
; Added jump for CONST mode switch to full 8 bit I/O mode.
; This is a non standard jmp call only for the Z80-MBC2
; --------------------------------------------------------
CONSTMD
    jp CNSTMODE_; Set CONST mode to full 8 bit
...
BOOT_
    ...
    ld      hl,BiosMsg      ; Print CBIOS message
    call    puts
    ld      hl, CPMMsg      ; Print CP/M 2.2 message
    call    puts            ;
    ;
...
WBOOT_
    ...
    ; CCP+BDOS are stored from host sector 0 to 10. 
    ; Total 11 host sectors (each 512 bytes large) to load
    ;
    ld      sp, $80         ; Use space below buffer for stack 
    ; Print a warm boot message if enabled (SETOPT Opcode)
    ld      a, SYSFLAG_OPC  ; Select SYSFLAG_OPC opcode (IOS)
    out     (STO_OPCD), a
    in      a, (EXC_RD_OPCD); Read it
    and     $10             ; Is warm boor message enabled (bit D4 = 1)?
    jr      z, noWarmMsg    ; No, jump
    ld      hl, WbootMSG    ; Yes, print a message
    call    puts 
noWarmMsg:
    ; Select host disk 0 and reset CONST mode
    ld      a, SELDISK_OPC  ; Select SELDISK opcode (IOS)
    out     (STO_OPCD), a
    xor     a               ; disk = 0
    ld      (cnstModeFlg), a; Reset the CONST mode (see CNSTMODE)
...
; CNSTMODE (CONST MODE)                                                       ;
; =========================================================================== ;
; This is not a standard CP/M routine but is Z80-MBC2 specific.               ;
; Calling CNSTMODE the CONST routine will work in full 8 bit mode.            ;
; After a cold or warm boot this will be reset.                               ;
; =========================================================================== ; 
CNSTMODE_
    ld      a, 1
    ld      (cnstModeFlg), a; Set the CONST mode flag to full 8 bit mode
    ret
    
cnstModeFlg
    .db     0               ; CONST mode flag. If = 1 the full 8 bit mode is active,
...
InChr                       ; Set char ready flag
    ld      a, $ff          ; Return CP/M a char ready flag ($FF)
    ret

CONST7
    ld      a, (InChrBuf)   ; A = previous char read by CONST, if any
    cp      $ff             ; Is = $FF ($FF from UART = no char)?
    jr      nz, InChr       ; No, jump (valid char already read and ready)
    in      a, (SERIAL_RX)  ; Yes, Read a char from "virtual" UART
...
NoInChr                     ; Set char not ready flag
    xor     a               ; A = 0
    ret                     ; Return CP/M a no char flag ($00)
    
InChrBuf                    ; Last read char by CONST ($FF = no char)
    .db     $ff             ; Initialized as $FF
...
CONIN_
    ...
    push    af              ; No, set InChrBuf = $FF (clear buffer)
    ld      a, $ff
    ld      (InChrBuf), a
    pop     af
    ;jr      SetChrPar
    ret                     ; Return with the previously saved char
...
; =========================================================================== ;
LIST_
    ld      a, GETSPP_OPC   ; Select GETSPP_OPC opcode (IOS)
    out     (STO_OPCD), a
    in      a, (EXC_RD_OPCD); Read it
    and     a               ; Is SPP enabled?
    jr      z, listRet      ; No, nothing to do so jump
    cp      SPPRDY          ; Yes, is SPP printer ready?
    jr      nz, LIST_       ; No, jump and wait
    ld      a, WRSPP_OPC    ; Yes, select WRSPP_OPC opcode (IOS)...
    out     (STO_OPCD), a
...
LISTST_
    ...
    cp      SPPRDY          ; Yes, is SPP printer ready?
    jr      nz, noSPP       ; No, jump
    ld      a, $ff          ; Yes, A = ready flag
    ret

noSPP:
    xor     a               ; A = 0 (not implemented/not ready flag)
    ret
...
BiosMsg     .db     cr, lf, lf, "Z80-MBC2 CP/M 2.2 BIOS - S030818-R120923", cr, lf, eos ;
